...
P Mode 默认使用 `backend/app/cognitive_planning` 下的 Cognitive OS 内核。LangGraph 只负责状态流转，独立角色负责真正判断；Phase 7.3 在 Goal Intelligence 后增加 Goal Completion Judge，并在任何 Goal Agent 调用前识别规划控制指令：
...
- **Goal Completion Judge** 判断现有语义是否足以继续；只有仍会改变计划的 blocking unknown 才能停止推进，optional unknown 会被保留但不会强迫用户补齐。
...
Phase 7.1 introduced one user-facing overview with friendly stage names and an opt-in technical trace. Phase 7.3 keeps that friendly stage mapping and Advanced Debug Mode boundary, while replacing the original collapsed-process presentation with the live Planning Workspace described below. Raw Agent names, handoffs, artifacts, model usage, routing, and fallback diagnostics still appear only after the persisted **Advanced Debug Mode** setting is enabled; it is off by default.

The additive `goal_understanding` NDJSON event is saved with the command thread and restored on replay. Phase 7.1 compatibility verification covers Beijing and Urumqi purpose clarification, a same-thread destination follow-up, the skiing/project consistency block, stream/replay preservation, friendly stage labels, and Advanced Debug Mode disclosure.

### Phase 7.3 / Planning State Progression Hotfix

Phase 7.3 is a completed hotfix inside the same Phase 7 boundary. It separates “the goal is sufficiently understood” from “the current model call succeeded,” so a transient strategy-model failure cannot send a complete goal back into clarification.

`GoalCompletionResult` is evaluated immediately after Goal Intelligence:

```ts
type GoalCompletionResult = {
  complete: boolean;
  blockingUnknowns: { question: string; impact: string }[];
  optionalUnknowns: string[];
  nextStage: "goal_clarification" | "evidence" | "strategy";
};
```

Completion is semantic, not a fixed `subject / purpose / duration` checklist. For example, the same planning session can receive “我要学go语言” → “为了web开发” → “学过python也做过web开发” → “找工作和个人项目” → “每周20小时”. Planix retains this as **Goal: Go Web development**, **Background: Python and Web development experience**, **Purpose: job search and personal projects**, and **Constraint: 20 hours/week**. A first-project deadline may remain optional; it does not block `complete=true` or `nextStage="strategy"`.

A Control Intent Router runs before Goal Intelligence. “下一步 / 继续 / 开始规划” maps to `continue_current_stage`, “确认” to `approve_current_stage`, “修改” to `modify_current_stage`, “重新开始” to `restart_planning`, and “取消” to `cancel_planning`; these phrases are not appended as new goal facts.

Planning progress is now exposed on two independent axes. `businessStatus` records durable progress such as `goal_understood`, `strategy_pending`, and `execution_pending`; `runtimeStatus` records execution health such as `running`, public `blocked_model`, or `retry_required`. For replay compatibility, a failed model call still has top-level `status="MODEL_UNAVAILABLE"` and `cognitiveMetadata.planningMode="blocked_model_unavailable"`. Once the model recovers, `continue_current_stage` resumes from the persisted failed stage—for example Strategy—without recollecting the goal or rerunning completed Goal, Reality, and Evidence artifacts.

Default P Mode now uses one live **Planning Workspace** card with **Current Stage**, **Goal Understanding**, **Known Facts**, **Important Unknowns**, and **Next Action**. It updates in place across user turns and replaces the old per-message “规划过程 · 已折叠” timeline. Advanced Debug Mode still provides the replayable technical cards without changing the default workspace.

On model failure, Planix may say that the goal model and known facts were saved and that it is waiting for the model. It must not generate a fake strategy, fake Agent decision, execution artifact, critique, or completed-planning claim. The additive `goal_completion_updated` event carries the judge result, while `planning_session_status` additively carries `businessStatus`, `runtimeStatus`, and `goalCompletion`; both remain compatible with existing command-thread replay.

SQLite startup migration adds `planning_sessions.goal_completion_json`, `business_status`, and `runtime_status` non-destructively and backfills old Planning Sessions from their existing status and artifacts. Phase 7.3 tests cover the multi-turn Go progression, control-intent routing, strategy `auth_error` preservation and same-stage recovery, additive stream/replay payloads, and the single live Planning Workspace.
...
用户输入目标后，P Mode 先区分规划控制指令与新的目标信息，再通过 FastAPI Planning Session 进入 Cognitive OS。Goal Completion Judge 只让 blocking unknown 阻止推进；optional unknown 不会把用户困在澄清阶段。每个认知阶段都使用模型和严格契约生成独立 artifact；规则只负责校验和 Gate，不生成规划内容。用户可以补充信息、继续失败前的阶段、比较并确认策略、审查执行蓝图，最后明确触发日历写入。

```text
用户目标
  → Control Intent Router
  → P Mode Planning Session
  → Goal Intelligence / Goal Completion Judge
  → Reality
  → Evidence / Strategy
  → 用户确认策略
  → Execution / Critic
...
P Mode 默认停留在 `auto` 状态。Phase 7.1 先运行模型支持的 `GoalUnderstandingResult`，再让普通查询、记忆和 Calendar 操作进入 `CommandDecision`；清晰规划请求直接进入 Cognitive Planning Session，模糊目标则先追问。Phase 7.3 在活动 Planning Session 中优先识别继续、确认、修改、重新开始和取消等控制指令，并让 Goal Completion Judge 按语义区分 blocking 与 optional unknown。启用 `PLANIX_COGNITIVE_MODE=true` 后，P Mode 依次形成目标理解、完整性判断、现实评估、证据包、策略方案、执行蓝图和独立审查。手动 `workbench` 继续保留旧 Runtime / hidden draft，仅用于兼容和调试。
...
```mermaid
flowchart TD
  U["User conversation"] --> CR["Control Intent Router"]
  CR --> G["Goal Intelligence Agent"]
  G --> GC["Goal Completion Judge"]
  GC -->|"blocking unknowns only"| Q["Wait for goal answer"]
  Q --> G
  GC -->|"complete; optional unknowns retained"| RY["Reality Agent"]
...
Formal planning is model-backed only. If the configured model is unavailable, returns invalid JSON, or cannot satisfy the typed contract, Planix preserves the last successful business stage and artifacts, exposes public `runtimeStatus="blocked_model"`, and keeps the compatibility status `MODEL_UNAVAILABLE`. It asks the user to retry or fix model settings, then resumes the failed stage instead of restarting goal collection. It does not silently present a local template, fake Agent decision, or fabricated artifact as an AI-created formal plan. Calendar remains blocked until the strategy and execution gates are explicitly approved and the independent critic marks the blueprint writable.

The P Mode workspace is replayable and user-facing: one live Planning Workspace updates Current Stage, Goal Understanding, Known Facts, Important Unknowns, and Next Action as Goal Completion, Reality, Evidence, Strategy, Execution, Critic Review, and Learning Update arrive. Raw Agent decisions, messages, confidence plumbing, and runtime status are retained for audit but hidden from the primary planning workspace. Existing Planning Session cards and old Runtime/draft history remain replay-compatible.
...
- Phase 7.3 Planning State Progression hotfix: semantic Goal Completion Judge, planning-control routing, durable `businessStatus` versus `runtimeStatus`, failed-stage recovery, additive completion/status replay, and one live Planning Workspace without per-message timelines.
