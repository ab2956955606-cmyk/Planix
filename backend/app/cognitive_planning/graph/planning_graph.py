...
def _after_goal(state: CognitivePlanningState) -> str:
    ...
    return "reality" if goal and goal.can_proceed_to_evidence else "wait_for_goal_answer"
...
def build_cognitive_os_graph(runtime):
    ...
    graph.add_conditional_edges("strategy", _after_strategy, {
        "execution": "execution", "wait_for_strategy_approval": "wait_for_strategy_approval",
    })
    ...
    graph.add_conditional_edges("critic", _after_critic, {
        "repair": "repair", "wait_for_execution_approval": "wait_for_execution_approval",
    })
